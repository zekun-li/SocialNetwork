...
def main():
	...
	for train_index, test_index in kf: 
		...
		xbest = []	# store the best feture matrix for traing section of traingX
		...
		for i in range(0, X.shape[1]):
			...
			for colInd in residual_col_indices:
				...
				if i == 0: # if it's the first feature to add
					xtemp = trainX_train[:,colInd].reshape(trainX_train.shape[0],-1) #convert to a column vector
				else: 
					xtemp = np.hstack((xbest, trainX_train[:,colInd].reshape(trainX_train.shape[0],-1) ))

				clf = LogisticRegression();
				clf.fit(trainX_train, trainy_train)

				y_true, y_pred = trainy_test, clf.predict(trainX_test)
			...
			if auc_best_global < auc_best_local : # if auc is increasing by adding new features
				if i == 0: # if it's the first feature to add
					xbest = trainX_train[:,colInd_best].reshape(trainX_train.shape[0],-1)
				else:
					xbest = np.hstack((xbest,trainX_train[:,colInd_best].reshape(trainX_train.shape[0],-1)))
		...
		print 'auc_best_global found on current trainX_test: '+str(auc_best_global)

	print '# features selected = '+str(len(feature_index)) 
	print 'feature_index = ' + str(feature_index)

	
	testy_true, testy_pred = testy, clf.predict(testX)
